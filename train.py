...
def main(_):
  ...
  audio_processor = AudioProcessor(FLAGS.data_url, FLAGS.data_dir, FLAGS.kaggle_test_data, model_settings)
  ...
  for i in xrange(0, set_size, batch_size):
      tf.logging.info('Progress: %.2f%%', float(100 * i) / float(set_size))
      test_fingerprints, test_ground_truth, noise_labels = audio_processor.get_data(
          batch_size, i, model_settings, 0.0, 0.0, 0, 'kaggle_test_data', sess)
      prediction = sess.run(
          graph.predicted_indices,
          feed_dict={
              graph.fingerprint_input: test_fingerprints,
              graph.is_training: 0,
              graph.dropout_prob: 1.0
